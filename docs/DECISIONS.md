...
| ID | Decision | Status | Rationale |
|---|---|---|---|
| U72-001 | Account deletion is self-only and irreversible (`evaluateAccountDeletion`): it anonymizes the public profile (`buildDeletionAnonymization` — "Deleted member", cleared about/answers/photos, hidden), closes matches (`closedReason:"deletion"`, counterpart sees "member left"), removes tokens/notifications, disables auth, and sets `users.status="deleted"`. `"deleted"` is a fourth account status and is terminal (staff cannot flip it back). | Implemented (Admin SDK/Cloudinary cascade deferred, O-001/O-002) | Master spec Section 7 / F11: KVKK/GDPR-compliant anonymization; deletion never leaves recoverable personal data. |
| U72-002 | Data export returns a privacy-safe bundle of the member's **own** data only (`assembleMemberExport`) — never another member's — downloaded as JSON. The UI requires a double confirmation (an explicit acknowledgement checkbox) before the irreversible delete. A deleted account is locked out of all product writes by the existing `isActive()` rule, same as suspended/banned. | Implemented (export CF deferred, O-001) | Master spec Section 7: verified, self-scoped privacy rights; destructive actions are never one click. |

### Unit 7.3 additions

| ID | Decision | Status | Rationale |
|---|---|---|---|
| U73-001 | The client reads the owner's own `users.status` (owner-readable) to route suspended / banned / deleted members to a `/[locale]/status` screen; all product surfaces stay behind the `isActive()` security rules, so this is UX-only (A-008). A missing/unknown status fails open on the client for smooth UX, never for authorization. | Implemented | Master spec 169/479: non-active members authenticate only to see their status; real lockout is server-side. |
| U73-002 | The `/status` screen is noindex and self-correcting — it bounces an unauthenticated visitor to login and an active member back to `/app` — so it can neither be indexed nor trap an eligible member, and it messages each status distinctly (temporary suspension vs permanent ban vs deleted). | Implemented | Master spec 322: a clear, honest, escape-safe status surface. |

### Unit 7.4 additions

| ID | Decision | Status | Rationale |
|---|---|---|---|
| U74-001 | Every response carries a self-first CSP and the standard security headers (HSTS, nosniff, DENY framing, Referrer-Policy, Permissions-Policy) via `next.config.ts`. The CSP allow-lists only Firebase/Cloudinary/reCAPTCHA origins; `script-src` keeps `'unsafe-inline'` (Next's inline bootstrap) with per-request nonces flagged as a production tightening in `docs/SECURITY.md`. | Implemented | Master spec «Security»: secure headers + strict CSP; honest about the one pragmatic relaxation. |
| U74-002 | App Check is invoked at client startup (`ensureAppCheck` in `AuthProvider`), so the attestation enforcement point exists in code and activates the moment a reCAPTCHA site key is configured — no code change needed at production wiring, only the key + server-side enforcement toggle (O-001). | Implemented (key/enforcement deferred, O-001) | Master spec: App Check as an anti-abuse enforcement point, wired but inert until keys land. |
| U74-003 | `docs/SECURITY.md` is the living threat model + hardening checklist; it records what is enforced (rules, redaction, limits, upload validation) and what is deferred (App Check enforcement, key rotation, CSP nonces, budgets/scanning), so the launch security review has a single source of truth. | Implemented | Master spec: a lightweight threat model before sensitive features / launch. |

### Unit 7.5 additions

| ID | Decision | Status | Rationale |
|---|---|---|---|
| U75-001 | `sitemap.xml` lists only the public marketing pages (with per-locale hreflang) and `robots.txt` disallows every private locale segment (app/admin/auth/onboarding/status); both derive from the `PUBLIC_PATHS` / `PRIVATE_PATH_SEGMENTS` allow/deny lists in `lib/seo.ts` so SEO surface and the crawl policy can't drift from the routes. `metadataBase` uses `NEXT_PUBLIC_SITE_URL` (placeholder until O-001). | Implemented | Master spec Section 7 launch: correct, localized, privacy-aware SEO. |
| U75-002 | 404s under a locale render a branded, localized RTL page via a `[locale]/[...rest]` catch-all → `[locale]/not-found.tsx`; a minimal bilingual root not-found covers non-locale paths. Error boundaries show a generic message (no stack/internal detail leaks) with retry — a localized `[locale]/error.tsx` and a dependency-free root `global-error.tsx`. | Implemented (client error reporting deferred, O-001) | Master spec: polished, honest, non-leaking error/empty states across the app. |
